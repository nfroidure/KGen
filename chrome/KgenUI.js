...
KgenUI.prototype.drawBottomBar = function()
	...
	this.bottombar.setAttribute('style','position:fixed; bottom:0; left:0; width:100%; height:235px; background:#dbdbdb; border-top:2px solid #ccc;');
	this.bottombar.innerHTML=''
		+'<p style="line-height:30px; font-size:15px; color:#fff; margin:0 1%;"><strong>KGen 4 Chrome : SEO Keyword Scanner</strong> - <a href="http://kgen.elitwork.com/">Home / Donate us</a></p>'
		...
		+'	</table>'
		+'</div>'
		+'<div id="kgenTagCloud" style="width:33%; float:left; margin:0 1% 0 1%; height:200px; overflow:auto; background:#fff; line-height:15px;">'
		+'</div>'
		+'<div id="kgenStats" style="width:22%; float:left; margin:0 1% 0 0; height:200px; overflow:auto; background:#fff;">'
		+'	<p style="line-height:15px; padding:0; margin:0;"><strong>Page stats</strong></p>'
		+'	<p>'
	...
	document.getElementById('kgenStatsTotalwords').innerHTML=this.currentKGen.wordsTot;
	document.getElementById('kgenStatsOriginalwords').innerHTML=this.currentKGen.words.length;
	document.getElementById('kgenStatsVocable').innerHTML=Math.round(this.currentKGen.words.length/this.currentKGen.wordsTot*10000)/100;
	document.getElementById('kgenStatsTotalchars').innerHTML=document.body.innerHTML.length;
	document.getElementById('kgenStatsEfficientchars').innerHTML=this.currentKGen.textChars;
	document.getElementById('kgenStatsHtmlquality').innerHTML=Math.round(this.currentKGen.textChars/document.body.innerHTML.length*10000)/100;
...
KgenKeywords.prototype.fill = function ()
	{
	this.empty();
	var withPercents = false;//this.kgenUI.currentOptions.getBoolOption('extensions.kgen.percent');
	var withWordsTot = false;//this.kgenUI.currentOptions.getBoolOption('extensions.kgen.wordstot');
	...
	for(var i=0; i<x; i++)
		...
		if(withPercents)
			td.innerHTML=this.kgenUI.currentKGen.words[i].getRepeatPercents((withWordsTot ? this.kgenUI.currentKGen.wordsTot : this.kgenUI.currentKGen.repeatTot));
		else
			td.innerHTML=this.kgenUI.currentKGen.words[i].repeat;
		tr.appendChild(td);
		td = document.createElement("td");
		if(withPercents)
			td.innerHTML=this.kgenUI.currentKGen.words[i].getWeightPercents(this.kgenUI.currentKGen.weightTot);
		else
			td.innerHTML=this.kgenUI.currentKGen.words[i].weight;
		tr.appendChild(td);
		td = document.createElement("td");
		if(withPercents)
			td.innerHTML=this.kgenUI.currentKGen.words[i].getAveragePositionPercents(this.kgenUI.currentKGen.wordsTot);
		else
			td.innerHTML=this.kgenUI.currentKGen.words[i].getAveragePosition();
		tr.appendChild(td);
		this.tbody.appendChild(tr);
		//curListitem.addEventListener('dblclick',this.kgenUI.newEventHandler(this.kgenUI.ignoredWordsListbox,this.kgenUI.ignoredWordsListbox.addItem),false);
		//curListitem.addEventListener('click',this.kgenUI.newEventHandler(this.kgenUI,this.kgenUI.suggestWord),false);
		}
	}
...
KgenKeywordTagCloud.prototype.fill = function ()
	{
	var factor=75;//this.kgenUI.currentOptions.getIntOption('extensions.kgen.cloud.factor');
	this.empty();
	var x = this.kgenUI.currentKGen.words.length;
	...
	for(var i=0; i<x; i++)
		...
		newSpan.setAttribute('style','font-size:'+ (this.kgenUI.currentKGen.words[i].getWeightPercents(this.kgenUI.currentKGen.weightTot)*factor/100) +'em; line-height:'+ (this.kgenUI.currentKGen.words[i].getWeightPercents(this.kgenUI.currentKGen.weightTot)*factor/600) +'em;');
