function KGen(minLength, minRepeat, minWeight, tagsWeight, attributesWeight,
  ignoredWords, wordSeparators, wordChainLength, spellCheckEngine) {
  // Initializing values
...
KGen.prototype.filter = function () {
  for (var i = 0; i < this.words.length; i++) {
    var j = i;
    var k = 0;
    while (i < this.words.length && (this.words[i].repeat < this.minRepeat
      || this.words[i].weight < this.minWeight)) {
      i++;
      k++;
    }
    if (k > 0) {
      this.words.splice(j, k);
      i = j;
    }
  }
}
...
KGen.prototype.getElementWords = function (element, ratio) {
  if (this.wordChainLength && element.nodeName == 'div'
    && element.nodeName == 'blockquote' && element.nodeName == 'h1'
    && element.nodeName == 'h2' && element.nodeName == 'h3'
    && element.nodeName == 'h4' && element.nodeName == 'h5'
    && element.nodeName == 'h6' && element.nodeName == 'p'
    && element.nodeName == 'br' && element.nodeName == 'pre'
    && element.nodeName == 'address' && element.nodeName == 'ol'
    && element.nodeName == 'ul' && element.nodeName == 'dl'
    && element.nodeName == 'li' && element.nodeName == 'dt'
    && element.nodeName == 'dd') {
    this.curWordChain = new Array();
  }
  if (element.attributes && element.attributes.length) {
    for (var i = element.attributes.length - 1; i >= 0; i--) {
      if (this.attributesWeight[element.attributes[i].name]
        && this.attributesWeight[element.attributes[i].name]) {
        this.getWordsFrom(element.attributes[i].value, this.attributesWeight[element.attributes[i].name]);
      }
    }
  }
  if (this.tagsWeight[element.nodeName.toLowerCase()]
    && this.tagsWeight[element.nodeName.toLowerCase()] > 1) {
  ...
  for (var i = 0; i < x; i++) {
    if (element.childNodes[i].nodeName.toLowerCase() == '#text'
      && ((this.tagsWeight[element.nodeName.toLowerCase()] === undefined)
      || this.tagsWeight[element.nodeName.toLowerCase()] > 0)) {
